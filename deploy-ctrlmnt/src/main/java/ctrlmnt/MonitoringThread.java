...
//    private static final Logger logger = LoggerFactory.getLogger(MonitoringThread.class.getName());
    ...
    static{
        logger = Logger.getLogger(MonitoringThread.class.getName());
    ...
    public void run() {
        ...
        logger.fine("requestCount : " + ControllableService.requestCount.get());
        logger.fine("requestCountM1 : " + ControllableService.requestCountM1.get());

        logger.fine("serviceTimeSum : " + ControllableService.serviceTimesSum.get());
        logger.fine("serviceTimeSumM1 : " + ControllableService.serviceTimesSumM1.get());
        try {
            ...
            logger.fine("rps : {}" + rps);
            logger.fine("avg_st : {}" + avg_st);
            writeCustomMetric("rps_gauge", rps);
            writeCustomMetric("st_gauge", avg_st);
        } catch (IOException e) {
            logger.severe("IOException in MonitoringThread: {}" + e.getMessage());
